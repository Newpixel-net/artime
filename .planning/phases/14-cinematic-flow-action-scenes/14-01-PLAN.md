...
must_haves:
  ...
  key_links:
    - from: "DialogueSceneDecomposerService.validateAndFixTransitions"
      to: "ShotContinuityService.checkJumpCut"
      via: "method call"
      pattern: "checkJumpCut"
...
Purpose: Implement FLOW-03 requirement - shots build cinematically on each other. Leverages existing ShotContinuityService.checkJumpCut() infrastructure but integrates into DialogueSceneDecomposerService where speech-driven shots are created.
...
# Key source files
@modules/AppVideoWizard/app/Services/DialogueSceneDecomposerService.php
@modules/AppVideoWizard/app/Services/ShotContinuityService.php (reference only - line 358 checkJumpCut, line 1163 getShotSize)
</context>
