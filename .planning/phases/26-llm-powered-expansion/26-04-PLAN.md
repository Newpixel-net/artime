...
3. Update buildHollywoodPrompt() to call buildTemplate() instead of build() (line 716):
```php
...
Verify build() delegates: grep for "return \$this->buildHollywoodPrompt" in StructuredPromptBuilderService.php
  </verify>
  <done>
build() method delegates to buildHollywoodPrompt(), all existing callers automatically get LLM expansion for complex shots, existing tests pass
  </done>
</task>
...
4. buildHollywoodPrompt() calls buildTemplate() (not build())
5. No circular method calls
</verification>

<success_criteria>
1. build() method delegates to buildHollywoodPrompt() - verified by grep
2. All 10 integration tests pass - verified by test run
3. ImageGenerationService (and all other callers of build()) automatically get LLM expansion for complex shots without code changes
4. Simple shots still use template path (complexity detection routes them through buildTemplate)
</success_criteria>
