...
must_haves:
  truths:
    ...
    - "User sees voice continuity warnings in wizard UI"
  artifacts:
    ...
    - path: "modules/AppVideoWizard/app/Livewire/VideoWizard.php"
      provides: "Voice continuity warning display"
      contains: "voiceContinuityIssues"
  key_links:
    ...
    - from: "VideoWizard"
      to: "VoiceContinuityValidator"
      via: "Calls validateAllScenes on scene generation"
      pattern: "VoiceContinuityValidator.*validateAllScenes"
---

<objective>
Create VoiceContinuityValidator service to detect voice drift between scenes and integrate it into the wizard workflow to display warnings to users.
...
should detect and report the issue, and the wizard should display these warnings to users.

Output: New VoiceContinuityValidator service with validateSceneTransition and
validateAllScenes methods, registered in service provider, integrated into VideoWizard
with UI warnings displayed to user.
</objective>
...
<task type="auto">
  <name>Task 3: Integrate VoiceContinuityValidator into wizard workflow</name>
  <files>modules/AppVideoWizard/app/Livewire/VideoWizard.php</files>
  <action>
Add voice continuity validation to the wizard workflow with user-facing warnings:

1. Add property to store validation issues (near other state properties):
```php
public array $voiceContinuityIssues = [];
```

2. Add method to validate voice continuity (call this after scene generation or when navigating between scenes):
```php
/**
 * Validate voice continuity across scenes and store issues for display (VOC-08).
 *
 * @return void
 */
public function validateVoiceContinuity(): void
{
...
3. Call validateVoiceContinuity() in the scene generation workflow. Find the method that generates scenes (e.g., generateScenes, buildSceneDNA, or similar) and add at the end:
```php
// Validate voice continuity after scene generation (VOC-08)
$this->validateVoiceContinuity();
```

4. Also call it in loadSceneMemory() after restoring scene data:
```php
...
```
  ...
  <verify>
Run: `grep -n "voiceContinuityIssues" modules/AppVideoWizard/app/Livewire/VideoWizard.php`
Should show:
- Property declaration
- Assignment in validateVoiceContinuity method

Run: `grep -n "validateVoiceContinuity" modules/AppVideoWizard/app/Livewire/VideoWizard.php`
Should show:
- Method definition
- At least one call site (in scene generation or loadSceneMemory)

Run: `grep -c "VoiceContinuityValidator" modules/AppVideoWizard/app/Livewire/VideoWizard.php`
Should return 1 or more (import and usage).
  </verify>
  <done>VoiceContinuityValidator integrated into wizard with voiceContinuityIssues available for UI display.</done>
</task>
...
Run: `grep -c "voiceContinuityIssues" modules/AppVideoWizard/app/Livewire/VideoWizard.php` - should return 2 or more
</verification>
...
- [ ] VideoWizard.voiceContinuityIssues property stores warnings
- [ ] validateVoiceContinuity() method integrates validator
- [ ] Warnings available for UI display after scene generation
</success_criteria>
