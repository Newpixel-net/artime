...
must_haves:
  truths:
    ...
    - "CharacterLookService provides expression presets as simpler alternative to full psychology"
  artifacts:
  ...
  key_links:
    ...
    - from: "CharacterLookService.php"
      to: "CharacterPsychologyService.php"
      via: "EXPRESSION_PRESETS is simpler preset system; getExpressionFromPsychology bridges to full service"
      pattern: "CharacterPsychologyService"
---
...
Note on EXPRESSION_PRESETS vs CharacterPsychologyService:
- EXPRESSION_PRESETS: Simpler preset system for common expressions when full psychology granularity isn't needed (quick selection)
- CharacterPsychologyService: Full emotion-to-physical mapping with intensity, subtext layers, Bible trait integration
- Bridge method `getExpressionFromPsychology()` allows using CharacterPsychologyService when more nuance is needed
</objective>
...
<task type="auto">
  <name>Task 2: Extend CharacterLookService with expression presets and psychology bridge</name>
...
1. Add import at top of file (for bridge method):
```php
...
```php
/**
 * CharacterPsychologyService for full emotion mapping when presets aren't enough
 */
protected ?CharacterPsychologyService $psychologyService = null;
```

3. Add EXPRESSION_PRESETS constant (after existing CHARACTER_DNA_TEMPLATES):
```php
/**
...
Place new methods after existing methods (before the private helper methods).
  ...
  <verify>CharacterLookService contains EXPRESSION_PRESETS constant, getExpressionPreset method, and getExpressionFromPsychology bridge method</verify>
  <done>CharacterLookService extended with 8 expression presets, builder method, and bridge to CharacterPsychologyService for complex cases</done>
</task>
...
4. CharacterLookService has getExpressionFromPsychology() bridge method
5. `buildAnchorDescription($character, 0)` returns string with wardrobe/hair details
...
- CharacterLookService provides bridge to CharacterPsychologyService for complex cases
