...
must_haves:
  truths:
    - "Anxious scenes include cramped framing and harsh shadows in generated prompts"
    - "Peaceful scenes include soft lighting and open space descriptions in generated prompts"
    - "Tense scenes (level 8+) show claustrophobic descriptors in prompt output"
    - "Environment overlay blends emotional mood with base location without losing location identity"
  artifacts:
...
<task type="auto">
  ...
  <action>
Create comprehensive unit tests with user-observable verification:

1. `test_getMiseEnSceneForEmotion_returns_all_components()` - Verify lighting, colors, space, atmosphere keys
2. `test_anxiety_produces_cramped_and_harsh_shadow_descriptors()` - Verify output contains "cramped" AND "shadow"
3. `test_peace_produces_soft_lighting_and_open_space_descriptors()` - Verify output contains "soft" AND "open"
...
6. `test_tension_level_1_describes_comfortable_space()` - Low tension = "comfortable" in output
7. `test_tension_level_10_describes_claustrophobic_space()` - High tension = "claustrophobic" or "oppressive" in output
8. `test_blendEnvironments_respects_intensity()` - 0.0 = pure base, 1.0 = pure emotional

Tests should verify actual string content in output to ensure user-observable results:
```php
public function test_anxiety_produces_cramped_and_harsh_shadow_descriptors()
{
    $service = new MiseEnSceneService();
    $result = $service->getMiseEnSceneForEmotion('anxiety');

    // User-observable: generated prompt will contain these descriptors
    $this->assertStringContainsString('cramped', strtolower($result['space']));
    $this->assertStringContainsString('shadow', strtolower($result['lighting']));
}

public function test_peace_produces_soft_lighting_and_open_space_descriptors()
{
    $service = new MiseEnSceneService();
    $result = $service->getMiseEnSceneForEmotion('peace');

    // User-observable: generated prompt will contain these descriptors
    $this->assertStringContainsString('soft', strtolower($result['lighting']));
    $this->assertStringContainsString('open', strtolower($result['space']));
}
```
...
Use PHPUnit patterns consistent with existing tests.
  ...
  <done>At least 6 unit tests cover emotion mapping, environmental blending, and tension scale with user-observable string assertions</done>
</task>
...
3. `getMiseEnSceneForEmotion('anxiety')['space']` contains "cramped"
4. `getMiseEnSceneForEmotion('peace')['lighting']` contains "soft"
5. `getSpacialTension(10)['space_modifier']` contains "claustrophobic" or "oppressive"
...
- Anxious emotion produces cramped/harsh descriptors visible in generated prompts
- Peaceful emotion produces soft/open descriptors visible in generated prompts
