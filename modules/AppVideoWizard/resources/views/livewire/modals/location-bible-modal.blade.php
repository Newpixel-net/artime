    ...
    <div class="vw-modal"
        ...
        @elseif(!empty($storyBible['locations']) && ($storyBible['status'] ?? '') === 'ready')
            <div style="padding: 0.35rem 1rem; background: rgba(16,185,129,0.1); border-bottom: 1px solid rgba(16,185,129,0.2); color: #6ee7b7; font-size: 0.65rem; display: flex; align-items: center; gap: 0.5rem;">
                <span style="color: #10b981;">{{ __('Synced') }}</span>
                {{ count($storyBible['locations']) }} {{ __('locations from Story Bible') }}
            </div>
        @endif
        ...
        <div style="flex: 1; overflow-y: auto; padding: 0.75rem; display: flex; gap: 0.75rem;">
            ...
            <div style="width: 200px; flex-shrink: 0; border-right: 1px solid rgba(255,255,255,0.1); padding-right: 0.75rem; overflow-y: auto;">
                ...
                @php
                    $locationsNeedingRefs = collect($locationBible['locations'] ?? [])->filter(function($loc) {
                        return empty($loc['referenceImageBase64']) && empty($loc['referenceImageStorageKey']) || ($loc['referenceImageStatus'] ?? '') !== 'ready';
                    })->count();
                @endphp
                ...
                {{-- Location Items List --}}
                @if(count($locationBible['locations'] ?? []) === 0)
                ...
                @else
                    <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                        @foreach($locationBible['locations'] ?? [] as $index => $location)
                            <div wire:click="editLocation({{ $index }})"
                                ...
                                <div style="width: 50px; height: 35px; border-radius: 0.35rem; overflow: hidden; flex-shrink: 0; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center;">
                                    ...
                                    @else
                                        <span style="font-size: 1rem; color: rgba(255,255,255,0.3);">{{ __('Loc') }}</span>
                                    @endif
                                </div>
            ...
            <div style="flex: 1; display: flex; flex-direction: column; gap: 0.5rem; overflow-y: auto;">
                @if(count($locationBible['locations'] ?? []) > 0)
                    @php
                        ...
                        $currentLocation = $locationBible['locations'][$editIndex] ?? null;
                    @endphp
                    ...
                    @if($currentLocation)
                        ...
                        <div style="display: flex; gap: 0.75rem;">
                                ...
                                <div style="width: 170px; height: 100px; background: rgba(0,0,0,0.3); border: 1px dashed rgba(139,92,246,0.3); border-radius: 0.375rem; display: flex; align-items: center; justify-content: center; overflow: hidden; position: relative;">
                                    ...
                                    @else
                                        <div style="text-align: center; color: rgba(255,255,255,0.4);">
                                            <div style="font-size: 1.5rem; margin-bottom: 0.2rem;">{{ __('Img') }}</div>
                            ...
                            <div style="flex: 1;">
                                ...
                                <div style="margin-bottom: 0.4rem;">
                                    ...
                                    <input type="text"
                                           wire:model.blur="locationBible.locations.{{ $editIndex }}.name"
                                ...
                                <div style="display: flex; gap: 0.35rem; margin-bottom: 0.4rem;">
                                    <div style="flex: 1;">
                                        ...
                                        <select wire:model.change="locationBible.locations.{{ $editIndex }}.type"
                                    ...
                                    <div style="flex: 1;">
                                        ...
                                        <select wire:model.change="locationBible.locations.{{ $editIndex }}.timeOfDay"
                                                style="width: 100%; padding: 0.3rem; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.15); border-radius: 0.25rem; color: white; font-size: 0.65rem;">
                                    ...
                                    <div style="flex: 1;">
                                        ...
                                        <select wire:model.change="locationBible.locations.{{ $editIndex }}.weather"
                                                style="width: 100%; padding: 0.3rem; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.15); border-radius: 0.25rem; color: white; font-size: 0.65rem;">
                                ...
                                <div style="margin-bottom: 0.4rem;">
                                    ...
                                    <select wire:model.change="locationBible.locations.{{ $editIndex }}.mood"
                                            style="width: 100%; padding: 0.3rem; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.15); border-radius: 0.25rem; color: white; font-size: 0.65rem;">
                        ...
                        <div style="margin-bottom: 0.4rem;">
                            ...
                            <textarea wire:model.blur="locationBible.locations.{{ $editIndex }}.description"
                        ...
                        <div style="margin-bottom: 0.4rem;">
                            ...
                            <input type="text"
                                   wire:model.blur="locationBible.locations.{{ $editIndex }}.lightingStyle"
                        ...
                        <div style="margin-bottom: 0.35rem;">
                            ...
                            @if($assignedScenesCount === 0 && $totalScenes > 0)
                                <div style="padding: 0.35rem 0.5rem; margin-bottom: 0.25rem; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 0.25rem;">
                                    <span style="color: #fca5a5; font-size: 0.55rem;">{{ __('Please assign at least one scene to this location') }}</span>
                                </div>
                            @endif
                            <div style="display: flex; flex-wrap: wrap; gap: 0.25rem; align-items: center;">
                                @foreach($scenes as $sceneIdx => $scene)
                                    @php
                        ...
                        <div x-data="{ stateOpen: false, newStateScene: '', newStateText: '' }" style="margin-top: 0.5rem;">
                            <button type="button"
                                ...
                                <span style="color: rgba(255,255,255,0.6); font-size: 0.6rem; transition: transform 0.2s;" :style="stateOpen ? '' : 'transform: rotate(-90deg)'">v</span>
                            ...
                            <div x-show="stateOpen" x-collapse>
                                ...
                                @if($hasAssignedScenes)
                                    ...
                                    <div style="display: flex; flex-wrap: wrap; gap: 0.25rem;">
                                        ...
                                        <button type="button" wire:click="applyLocationStatePreset({{ $editIndex }}, 'weather-change')" style="padding: 0.2rem 0.4rem; background: rgba(6,182,212,0.1); border: 1px solid rgba(6,182,212,0.2); border-radius: 0.25rem; color: #67e8f9; font-size: 0.55rem; cursor: pointer;">{{ __('Weather') }}</button>
                                        <button type="button" wire:click="applyLocationStatePreset({{ $editIndex }}, 'abandonment')" style="padding: 0.2rem 0.4rem; background: rgba(107,114,128,0.2); border: 1px solid rgba(107,114,128,0.3); border-radius: 0.25rem; color: #9ca3af; font-size: 0.55rem; cursor: pointer;">{{ __('Abandon') }}</button>
                ...
                @else
                    ...
                    <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; color: rgba(255,255,255,0.4);">
                        <span style="font-size: 2.5rem; margin-bottom: 0.5rem;">{{ __('Loc') }}</span>
        ...
        <div style="padding: 0.5rem 1rem; border-top: 1px solid rgba(255,255,255,0.1); display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;">
            ...
            <div style="display: flex; gap: 0.4rem; align-items: center;">
                <label style="display: flex; align-items: center; gap: 0.3rem; cursor: pointer; font-size: 0.65rem; color: rgba(255,255,255,0.7);">
                    <input type="checkbox" wire:model.live="locationBible.enabled" style="accent-color: #f59e0b;">
                    {{ __('Enable') }}
                </label>
                <button type="button"
